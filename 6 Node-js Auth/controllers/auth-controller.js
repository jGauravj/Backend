  ...
  try {
    ...
    if (checkExistingUser) {
      res.status(400).json({
        success: false,
        message:
          "User is already exist either with same usename or same email. Please try with diffrent username or email",
      });
    }
    ...
    if (newlyCreatedUser) {
      ...
      res.status(400).json({
        success: false,
        message: "Unable to registerd user. Please try again!!",
      });
    }
  } catch (e) {
    console.log("Error->", e);
